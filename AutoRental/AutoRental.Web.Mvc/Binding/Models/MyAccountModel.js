(function (ar) {
    var MyAccountModel = function (accountId, loginEmail, firstName,
        lastName, address, city, state, zipCode, creditCard, expDate) {
        ...
        self.ExpDate = ko.observable(expDate.substring(0, 2) + "/" + expDate.substring(2, 4)).extend({
            ...
            pattern: {
                message: "Expiration date is an invalid format (must be MM/YY)",
                params: /^(0[1-9]|1[0-2])\/[0-9]{2}$/
            }
        });
    }
    ar.MyAccountModel = MyAccountModel;
}(window.AutoRental));
