...
window.AutoRental = {}; // oco: create & attach empty AutoRental object to global client/browser scope, ie. window

(function (ar) {
    var initialId;
    ar.initialId = initialId;
}(window.AutoRental)); // pass AutoRental (alias: ar) into this iife 
...
(function (ar) {
    var viewModelHelper = function () {
        ...
        self.apiGet = function (uri, data, success, failure, always) {
            ...
            $.get(AutoRental.rootPath + uri, data)
                .done(success)
        ...
        self.apiPost = function (uri, data, success, failure, always) {
            self.isLoading(true); // turn on loading indicator
            self.modelIsValid(true);
            $.post(AutoRental.rootPath + uri, data)
                .done(success)
                .fail(function (result) {
                    if (failure == null) {
                        if (result.status != 400)
        ...
        self.pushUrlState = function (code, title, id, url) {
            self.stateInfo = { State: { Code: code, Id: id }, Title: title, Url: AutoRental.rootPath + url };
        }
    ...
    ar.viewModelHelper = viewModelHelper;
}(window.AutoRental));

//oco: add custom binding to ko.bindingHandlers
